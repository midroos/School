...
class FinanceSystem:
    ...
    def get_daily_stats(self):
        ...
        today_date = datetime.now().strftime("%Y-%-d")
    ...
    def create_fee_plan(self, student_id, total_fees, installments_count, start_date_str):
        ...
        try:
            start_date = datetime.strptime(start_date_str, "%Y-%-d")
        except ValueError:
        ...
        try:
            for i in range(installments_count):
                due_date = (start_date + timedelta(days=30 * i)).strftime("%Y-%-d")
    ...
    def add_new_student(self, name, grade, academic_year, parent_phone):
        ...
        try:
            cur.execute("""INSERT INTO students (name, grade, academic_year, parent_phone, created_at)
                           VALUES (?, ?, ?, ?, ?)""",
                           (name, grade, academic_year, parent_phone, datetime.now().strftime("%Y-%-d")))
            conn.commit()
